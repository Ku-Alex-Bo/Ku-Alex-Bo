<h1 align="center">Hi, I'm Aleksandr Kuznetsov 👋</h1>

<p align="center">
  ⚙️ Web Backend · 🐍 Python / Django <br>
  🤖 Telegram Bots · Aiogram <br>
  🕷️ Parsing · BS4 / Selenium / Scrapy
</p>

---

## 💼 About Me

- 🔭 I work in a sports analytics company. Also freelancing and contributing to startup and open-source projects.
### 🧰 Tech Stack
- **Main Language**: `Python`
- **Also Using**: `JavaScript`, `C`
- **Frameworks**: `Django`, `DRF`, `Aiogram`
- **Databases**: `PostgreSQL`, `SQLite`, `Redis`
- **Async / Tasks**: `Celery`
- **Deployment**: `Docker`, `Nginx`, `Gunicorn`
- **OS**: `Linux (Ubuntu)`
- **Other**: `Git`, `pytest`, `Sphinx`
---

## 🧠 My Projects

| Name | Description | Stack |
|------|-------------|-------|
| **Serbuddy**  | Backend API and Telegram bot for Serbian language learning ([Demo](https://t.me/SerBuddyBot)) | Django, DRF, PostgreSQL, Redis, Celery, Aiogram, Docker |
| **SportArea** | Full-stack project: database of sport sources ([Demo](https://sportarea.online)) | Django, DRF, PostgreSQL, Redis, Celery, Docker, Vue3 |

---

## 🌐 Socials

[![Telegram](https://img.shields.io/badge/Telegram-blue?style=flat&logo=telegram)](https://t.me/NightJuly)
